...
<tbody>
<?php
$sql = "SELECT o.*, c.customer_id, c.customer_name, a.ad_id, a.ad_name, oi.cart_id,oi.product_id, oi.item_status,
(SELECT COUNT(*) FROM order_items WHERE cart_id = oi.cart_id) AS total_items,
(SELECT COUNT(*) FROM order_items WHERE cart_id = oi.cart_id AND item_status = 'Completed') AS completed_items, oi.item_status,
(SELECT COUNT(*) FROM order_items WHERE cart_id = oi.cart_id AND (item_status = 'Cancelled' OR item_status = 'Cancelling')) AS cancelled_items
FROM orders o
INNER JOIN order_items oi ON oi.cart_id = o.cart_id
INNER JOIN customers c ON c.customer_id = o.customer_id
INNER JOIN admin a ON a.ad_id = o.seller
GROUP BY oi.cart_id ORDER BY o.order_date DESC";
...
<td>
<?php $order_date = date("d/m/Y", strtotime($row['order_date']));echo $order_date;?>
</td>
<td><?php echo $row['customer_name']; ?></td>
<td><?php echo $row['ad_name']; ?></td>
<td><?php echo $total_items; ?></td>
<td><span class="badges <?php if ($row['status'] == 'Completed' AND $total_items >= 1 AND $row['item_status'] == 'Completed' ) {echo ' bg-lightgreen';}else{echo ' bg-lightred';} ?>"><?php echo $row['item_status']; ?></span></td>
<td>
<?php
 echo '<span class="badges bg-lightgreen"> Completed : ' . $completed_items. '</span>';
?>
</td>
<td>
<?php
echo '<span class="badges bg-lightred"> Cancelled : ' . $cancelled_items. '</span>';
    ?>
</td>
<td><span class="badges bg-lightgreen"><?php echo $row['transaction_payement']; ?></span></td>
...
<li>
<a href="invoice.php?cart_id=<?php echo $row['cart_id']; ?>" class="dropdown-item"><img src="assets/img/icons/download.svg" class="me-2" alt="img">Download pdf</a>
</li>
<li>
